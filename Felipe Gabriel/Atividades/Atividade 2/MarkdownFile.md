| Cor | Hexadecimal | Justificativa |
| --- | --- | --- |
| Primária | #4A90E2 | A cor azul traz confiança |
| secundaria | #50E3c2 | O verde água traz calma |
| Destaque | #F5A623 | O laranja chama a atenção |
| Fundo | #F8F8F8 | O fundo claro traz um clima limpo |
|  |  |  |
| Texto | #333333 | O texto cinza garante legibilidade |

1.  Paletas de cores  
    
2.  Tipografia

A Montserrat, com suas geométricas e peso moderno, confere aos títulos uma presença forte e amigável, alinhando-se a uma identidade visual contemporânea. Sua clareza em tamanhos maiores a torna ideal para hierarquizar o conteúdo. A Open Sans, por sua vez, é amplamente reconhecida por sua excelente legibilidade em parágrafos e textos corridos, mesmo em tamanhos menores. Sua neutralidade e familiaridade garantem que a informação seja facilmente absorvida pelo usuário, sem distrações, complementando perfeitamente a força da Montserrat.

3\. Estudo de Guidelines: Material Design

Princípio Selecionado: Movimento Significativo (Meaningful Motion)

O Material Design, criado pelo Google, enfatiza que o movimento em interfaces não deve ser meramente decorativo, mas sim funcional e significativo. Ele serve para guiar o usuário, fornecer feedback visual, indicar relações entre elementos e ajudar a manter o contexto. O movimento deve ser previsível e natural, imitando as leis da física para criar uma experiência tangível e intuitiva. Isso inclui animações suaves em transições de tela, respostas a interações do usuário (como pressionar um botão) e a indicação de hierarquia visual. O objetivo é tornar a interface mais viva, responsiva e agradável de usar, sem sobrecarregar o usuário com excesso de animação.